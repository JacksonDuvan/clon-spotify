...
class MainPlaylist extends React.Component{
    ...
    render(){
        ...
        const { playlistTracks:{ tracks :{ items }} } = this.props

        return(
            <div className="main-tracks">
                <div className="main-tracks__img">
                    <img src={playlistTracks.images[0].url} width="250" height="250" alt=""/>
                    <h1>{playlistTracks.name}</h1>
                </div>
                <div className="list-tracks">
                    {
                        items.map(item => (
                            <Link to={`/playlist/track/${id}/${item.track.id}`} className="track" onClick={() => this.handleClick(item.track.id)}>
                                     <Tracks
                                        id={item.track.id}
